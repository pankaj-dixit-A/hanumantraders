...
public static class clsCommon
{
    ...
    public static double getclosingstockbalance(int companycode, int yearcode)
    {
        ...
        using (SqlConnection con = new SqlConnection(strcon))
        {
            ...
            string XML = "<ROOT><Head Company_Code='" + companycode+ "' year='" + yearcode + "' upto_date='" + uptodate + "'></Head></ROOT>";
        ...
        if (dtnew.Rows.Count > 0)
        {
            ...
            {
                DataTable results = dtnew.Select("Brand_Code =" + dt.Rows[i]["Brand_Code"] + " and  Wt_Per=" + dt.Rows[i]["Wt_Per"]).CopyToDataTable();
                 rate = Convert.ToDouble(results.Rows[0]["Rate"].ToString());
                dt.Rows[i]["Rate"] = rate;
                rateper = dt.Rows[i]["RatePer"].ToString();
                if (rateper == "K")
                {
                    balance=Convert.ToDouble( dt.Rows[i]["balance"]);
                    wtper = Convert.ToDouble(dt.Rows[i]["Wt_Per"]);
                    closingbalance = closingbalance+(balance * wtper / 100) * rate;
                }
                else 
                {
                    balance = Convert.ToDouble(dt.Rows[i]["balance"]);
                    wtper = Convert.ToDouble(dt.Rows[i]["Wt_Per"]);
                    closingbalance = closingbalance + (balance * rate);
                }
            }
        }
